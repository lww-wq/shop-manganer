<template>
	<div class="page">
		<p-page-header title="优惠券类型管理"></p-page-header>
...
<script>
	...
	export default{
		...
		computed:{
			// 将vuex中定义的couponTypeModel模块的数据加载到本页的计算属性中，这样我们在本页就可以
			// 直接通过this.list和this.page访问到vuex中的数据，这里的数据只能读取不能写入
			...mapState('couponTypeModel',['list','page']),
			// 格式化日期的计算属性
			formatTime(){
		...
		methods:{
			...mapActions('couponTypeModel',['getListForPage','deleteById','setFreeze']),
			...
			// 跳转到添加数据的页面
			handleAdd(){
				this.$router.push('/coupon-type-add')
			},
			// 删除数据调用的方法
			...
			// 跳转到编辑用户的页面
			handleEdit(id){
				this.$router.push({path:'/coupon-type-edit',query:{id}})
			},
